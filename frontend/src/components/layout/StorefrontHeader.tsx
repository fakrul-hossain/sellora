        ...
        <form onSubmit={handleSearchSubmit} className="flex-1 max-w-2xl relative">
          <div className="flex items-center rounded-xl bg-slate-100/90 border border-slate-200 focus-within:border-brand-primary focus-within:bg-white focus-within:ring-2 focus-within:ring-brand-primary/20 transition-all">
            ...
            <input
              ...
              placeholder="Search products in Sellora database..."
            ...
            <button
              type="submit"
              className="bg-brand-primary hover:bg-brand-primary-hover text-white font-extrabold text-xs sm:text-sm px-6 py-2 rounded-r-xl transition-colors shrink-0 cursor-pointer"
            >
              Search
            </button>
          </div>
          ...
          <AnimatePresence>
            {isSearchFocused && searchQuery.trim() && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 6 }}
                className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl border border-slate-200 shadow-xl z-50 p-4 space-y-2 max-h-96 overflow-y-auto"
              >
                      ...
                      {searchResults.slice(0, 6).map((item) => (
                        ...
                        >
                          <img
                            src={item.imageUrl}
                            alt={item.title}
                            className="w-10 h-10 object-contain rounded-md border border-slate-100 bg-white p-0.5 shrink-0"
                          />
