...
import { Star, Heart, ShoppingBag, ShieldCheck } from 'lucide-react';
...
export const CompactProductCard: React.FC<CompactProductCardProps> = ({ product }) => {
  ...
  return (
    <div className="group relative bg-white rounded-2xl p-3 border border-slate-200/80 hover:border-brand-accent/50 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col justify-between h-full">
      {/* Top Image Container */}
      <Link href={`/products/${product.id}`} className="relative aspect-square w-full rounded-xl bg-slate-50/80 p-3 flex items-center justify-center overflow-hidden block">
        {/* Discount Badge */}
      ...
      <div className="mt-3 space-y-1.5 flex-1 flex flex-col justify-between">
        <div>
          ...
          <Link href={`/products/${product.id}`}>
            <h3 className="text-xs font-bold text-brand-primary line-clamp-2 mt-0.5 leading-snug hover:text-brand-accent transition-colors">
              {product.title}
            </h3>
          </Link>
        </div>
        ...
        <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
          ...
          <button
            ...
            className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all shadow-sm cursor-pointer ${
              isAdded
