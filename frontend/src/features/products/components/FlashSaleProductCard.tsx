...
export const FlashSaleProductCard: React.FC<FlashSaleProductCardProps> = ({ product }) => {
  ...
  return (
    <div className="group relative bg-white rounded-2xl p-4 border border-brand-accent/30 hover:border-brand-accent shadow-sm hover:shadow-lg transition-all flex flex-col justify-between h-full min-w-[240px]">
      <Link href={`/products/${product.id}`} className="relative aspect-square w-full rounded-xl bg-slate-50 p-4 flex items-center justify-center overflow-hidden block">
      ...
      <div className="mt-3 space-y-2 flex-1 flex flex-col justify-between">
        <div>
          ...
          <Link href={`/products/${product.id}`}>
            <h3 className="text-xs font-bold text-brand-primary line-clamp-2 mt-0.5 hover:text-brand-accent transition-colors">
              {product.title}
            </h3>
          </Link>
        </div>
        ...
        <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={handleAdd}
            className="px-3 py-1.5 rounded-xl bg-brand-accent text-white font-bold text-xs hover:bg-brand-accent/90 transition-colors shadow-sm flex items-center space-x-1 cursor-pointer"
          >
